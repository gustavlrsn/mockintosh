...
class EscPosEncoder {
   ...
   *
   */
  image(imageData, width, height, algorithm, threshold) {
    if (this._embedded) {
    ...
    /// let image = canvasContext.getImageData(0, 0, width, height);
    //let image = canvasData;
    let image = Flatten.flatten(imageData, [0xff, 0xff, 0xff]);
    console.log({ image });
    switch (algorithm) {
      case "threshold":
