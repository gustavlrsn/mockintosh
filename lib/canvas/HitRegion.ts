export interface HitRegionMask {
  data: Uint8Array;
  width: number;
  height: number;
}
...
export interface HitRegion {
  ...
  w: number;
  h: number;
  /** Per-pixel mask — only non-zero pixels are clickable. */
  hitMask?: HitRegionMask;
  cursor?: string;
  onMouseEnter?: () => void;
  onMouseLeave?: () => void;
  ...
  /** Local to the hit rect, then global screen (matches mousedown / click). */
...
export class HitRegionMap {
    ...
    for (let i = this.regions.length - 1; i >= 0; i--) {
      ...
      if (x >= r.x && x < r.x + r.w && y >= r.y && y < r.y + r.h) {
        if (r.hitMask) {
          ...
          if (lx < 0 || lx >= r.hitMask.width || ly < 0 || ly >= r.hitMask.height) continue;
          if (!r.hitMask.data[ly * r.hitMask.width + lx]) continue;
        }
        return r;
      }
    }
  ...
  handleMouseMove(x: number, y: number): boolean {
    ...
    if (this.pressedId !== null) {
      ...
      if (pressed?.onDrag) {
        pressed.onDrag(x - pressed.x, y - pressed.y, x, y);
      }
      return true;
    }
