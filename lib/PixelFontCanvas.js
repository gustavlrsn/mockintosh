export default class PixelFontCanvas {
  ...
  static drawText(canvas, text, style = {}) {
    style = {
      ...
      width: style.width !== undefined ? Math.round(style.width) : undefined,
      // width:
      ...
      bg: style.bg !== undefined ? style.bg : null,
      height:
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      // Check if the next character will cause a line wrap
      const nextX = (pos.x + charData.xAdvance) * style.scale;

      if (
        lastSpace !== -1 &&
        style.width > 0 &&
        nextX > style.width - 2 * style.padding
      ) {
    ...
    for (let i = 0; i < lenChars; i++) {
      let c = {
        position: { x: 0, y: 0 },
        rect: chars[i].charData.textureRect,
        scale: { x: 0, y: 0 },
        tint: "#FFFFFF",
      };
      _glyphs.push(c);
      c.position.x =
        chars[i].position.x * scale + lineAlignOffsets[chars[i].line];
    ...
    drawData.bg = style.bg;
    drawData.height = style.height;
    drawData.width = style.width;
  ...
  static _renderText(canvas, drawData, glyphs) {
    ...
    if (PixelFontCanvas._texture_cache[drawData.font + "-" + drawData.tint]) {
      var el =
        PixelFontCanvas._texture_cache[drawData.font + "-" + drawData.tint];
    } else {
    ...
    let minX = Infinity,
      minY = 0,
      maxX = -Infinity,
      maxY = drawData.height;
    glyphs.forEach((g) => {
      minX = Math.min(minX, g.position.x);
      maxX = Math.max(maxX, g.position.x + g.rect.width * g.scale.x);
    });

    // Set canvas width to text width if not set
    if (!drawData.width) {
      canvas.width = maxX;
    }
