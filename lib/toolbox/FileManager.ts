...
export function getIconForNode(node: FSNode): string {
  ...
  if (node.kind === "directory") {
    if (node.name === "Trash") return "icon/trash";
    return "icon/folder";
  }
...
export class FileManager {
  ...
  /** Set the node's zOrder higher than every other node so it renders on top. */
  bumpZOrder(nodeId: string): void {
    const node = this.meta.nodes[nodeId];
    if (!node || nodeId === ROOT_ID) return;
    let maxZ = 0;
    for (const n of Object.values(this.meta.nodes)) {
      if (n.zOrder != null && n.zOrder > maxZ) maxZ = n.zOrder;
    }
    node.zOrder = maxZ + 1;
    this.schedulePersist();
    this.notify();
