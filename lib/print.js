...
export const authClickHandler = async function (event) {
  const devices = await navigator.usb.getDevices();
  console.log({ devices });
  if (devices.length) {
    if (devices[0].opened === false) {
...
export const printClickHandler = async (imageData, width, height) => {
  ...
  const dang = encoder
    ...
    .image(imageData, imageData.width, imageData.height)
  ...
  await sendBytes(window.device, dang);
